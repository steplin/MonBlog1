...
class Router
{
		...
		if (isset($routes)) {
			foreach ($routes as $route) {
				...
				if ($routeMatch->match($this->request->requestURL())) {
					...
					if ($this->route->app() == 'Backend' && !$this->session->isAuthenticated()
					) {
						$this->response->redirect('../connexion.html');
					}

					if ($this->route->app() == 'Backend' && $this->session->isAuthenticated() && $this->session->session('role') != 1
					) {
						$this->response->redirect('../');
					}
				}
	...
	public function getController(Route $route)
	{
		...
		return new $controller(
			$this->response, $route->app(), $route->action(), $this->session
		);
	}
}
